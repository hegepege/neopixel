...
#define NUMLEDS        24              // Use total of 2 LEDs
...
void loop() {
  for(int j=0; j<NUMLEDS-1; j++){
    for(int i=0; i<255; i++) {            // Count up color units from 0 to 255
      leds.setPixelColor(j, leds.Color(i, 0, 0));  // Color mix for LED0
      leds.setPixelColor(j+1, leds.Color(0, i, 0));  // Color mix for LED1
      leds.show();                        // Set the LEDs
      delay(wait);                        // Delay for visual effect
    }
    for(int i=0; i<255; i++) {            // Loop through next color change
      leds.setPixelColor(j, leds.Color(0, i, 0)); 
      leds.setPixelColor(j+1, leds.Color(0, 0, i)); 
      leds.show(); 
      delay(wait); 
    }
    for(int i=0; i<255; i++) {            // Loop through next color change
      leds.setPixelColor(j, leds.Color(0, 0, i));
      leds.setPixelColor(j+1, leds.Color(i, 0, 0)); 
      leds.show(); 
      delay(wait);
    }
